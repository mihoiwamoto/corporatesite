// Orb Background using OGL
// Converted from React Bits Orb component to Vanilla JS
...
class OrbBackground {
  constructor(container, options = {}) {
    ...
    this.options = {
      hue: options.hue || 303,
      hoverIntensity: options.hoverIntensity || 1.65,
      rotateOnHover: options.rotateOnHover !== undefined ? options.rotateOnHover : false,
  ...
  init() {
    ...
    this.renderer = new OGL.Renderer({ alpha: true, premultipliedAlpha: false });
...
// Initialize function
function initOrb() {
  const orbContainer = document.querySelector('.ai-section-wrapper .orb-container');
  if (orbContainer) {
    console.log('Initializing Orb background...');
    console.log('OGL available:', typeof OGL !== 'undefined');
    ...
    try {
      ...
      console.error('Error initializing Orb background:', error);
    }
  } else {
    console.warn('Orb container not found');
  }
}
...
if (document.readyState === 'loading') {
  ...
  // DOM already loaded, but wait a bit for OGL to be ready
